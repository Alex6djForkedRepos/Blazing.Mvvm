...
This analyzer detects when a Blazor component uses [`MvvmComponentBase<TViewModel>`](xref:Blazing.Mvvm.Components.MvvmComponentBase`1) but the ViewModel depends on scoped services (such as Entity Framework `DbContext`). In such cases, the component should inherit from [`MvvmOwningComponentBase<TViewModel>`](xref:Blazing.Mvvm.Components.MvvmOwningComponentBase`1) instead to create a service scope and ensure proper lifetime management of scoped dependencies.

## Problem

When a ViewModel injects scoped services like `DbContext`, using the standard [`MvvmComponentBase`](xref:Blazing.Mvvm.Components.MvvmComponentBase`1) can lead to:
...
Use [`MvvmOwningComponentBase<TViewModel>`](xref:Blazing.Mvvm.Components.MvvmOwningComponentBase`1) which creates a dedicated service scope for the component and its ViewModel, ensuring:
...
|----------|-----------|--------|
| ViewModel uses only transient/singleton services | [`MvvmComponentBase`](xref:Blazing.Mvvm.Components.MvvmComponentBase`1) | No service scope needed |
| ViewModel injects `DbContext` or database connections | [`MvvmOwningComponentBase`](xref:Blazing.Mvvm.Components.MvvmOwningComponentBase`1) | Creates owned scope for proper disposal |
| ViewModel uses scoped services | [`MvvmOwningComponentBase`](xref:Blazing.Mvvm.Components.MvvmOwningComponentBase`1) | Ensures proper service lifetime |
