...
For messenger-only scenarios, consider using [`RecipientViewModelBase`](xref:Blazing.Mvvm.ComponentModel.RecipientViewModelBase) which handles messenger cleanup automatically:
...
- Consider [`RecipientViewModelBase`](xref:Blazing.Mvvm.ComponentModel.RecipientViewModelBase) for messenger-only scenarios
