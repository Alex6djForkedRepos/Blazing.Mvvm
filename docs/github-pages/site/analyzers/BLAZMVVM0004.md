...
This analyzer ensures that [`[ViewParameter]`](xref:Blazing.Mvvm.ComponentModel.ViewParameterAttribute) attributes are only used on ViewModel properties that have corresponding `[Parameter]` properties in the associated component. This maintains proper data flow between Views and ViewModels.
