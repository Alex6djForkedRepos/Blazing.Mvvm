...
Blazor components with ViewModel properties should inherit from [`MvvmComponentBase<TViewModel>`](xref:Blazing.Mvvm.Components.MvvmComponentBase`1) to ensure proper View-ViewModel binding.

## Description

This analyzer detects Blazor components that have a ViewModel property but don't inherit from [`MvvmComponentBase<TViewModel>`](xref:Blazing.Mvvm.Components.MvvmComponentBase`1). Using the proper base class ensures automatic ViewModel injection, lifecycle management, and proper state synchronization.

## Severity

**Error** - This is a critical issue that will prevent proper View-ViewModel binding.
...
1. Replaces `ComponentBase` with [`MvvmComponentBase<TViewModel>`](xref:Blazing.Mvvm.Components.MvvmComponentBase`1)
