                ...
                hasAnyUnregisterCall = classDeclaration.DescendantNodes()
                    ...
                    .Any(invocation =>
                    {
                        var identifierText = invocation.Expression.ToString();
                            return ((identifierText.Contains("Messenger") || identifierText.Contains("WeakReferenceMessenger")) &&
                                    identifierText.Contains("Unregister")) ||
                               identifierText.Contains("UnregisterAll");
                    });

                if (hasAnyUnregisterCall)
                {
                    break; // Has explicit cleanup mechanism, no need to check further
                }
            }
        }

        // If class has explicit cleanup mechanism, no diagnostic needed.
        // Inherited IDisposable from ViewModelBase is not enough on its own.
        if (hasAnyUnregisterCall)
        {
            return;
        }
